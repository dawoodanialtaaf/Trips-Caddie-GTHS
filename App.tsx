...
const App: React.FC = () => {
  ...
  const refreshData = async () => {
    const data = await Storage.getRecaps();

    // Sort by newest year + newest month
    const monthOrder = [
  ...
  const availableTripTypes = useMemo(() => {
    const types: string[] = [];
    if (recaps.some(r => r.rounds > 0)) types.push('golf');
    if (recaps.some(r => r.rounds === 0)) types.push('charter');
    return types;
  }, [recaps]);
              ...
              <div className="flex flex-col xl:flex-row items-center justify-between gap-8 bg-white p-8 rounded-2xl shadow-sm border border-slate-200/60">
                <div className="max-w-3xl">
                  ...
                  <p className="text-slate-600 text-sm leading-relaxed">
                    These are just some of the trips that we’ve booked for actual customers all over the world.
                    Please be advised that rates vary depending on many factors, so please use this tool to find
                    the trip that fits your budget and the destination you’d like to play in and then call us to
