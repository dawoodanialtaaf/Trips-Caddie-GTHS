...
import { X, Calendar, MapPin, Trash2, Search, Monitor, Clock, Download, ChevronDown, ChevronUp, Globe, Info } from 'lucide-react';
...
const AdminRequestLogModal: React.FC<AdminRequestLogModalProps> = ({ logs, onDeleteLog, onClose }) => {
  ...
  const handleExportCSV = () => {
      const headers = [
          'Date', 'Time', 'Customer Name', 'Email', 'Phone', 'Company', 
          'Trip Name', 'Request Notes', 
          // Trip Details Columns
          'Trip Month/Year', 'Group Size', 'Lodging', 'Courses', 'Price/Person', 'Vibe', 'Transport', 'Special Requests', 'Highlights', 'Synopsis', 'Planner Notes', 'Itinerary Summary',
          // Metadata Columns
          'City', 'Region', 'Country', 'IP', 'Device', 'Source'
      ];
      ...
      const csvContent = [
        ...
        ...filteredLogs.map(log => {
          ...
          const itineraryStr = snap?.dailyItinerary
            ? snap.dailyItinerary.map(d => `Day ${d.day}: ${d.activity} @ ${d.time} (${d.location})`).join(' | ') 
            : '';
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full flex flex-col h-[85vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-xl">
          <div>
            ...
            <p className="text-xs text-slate-500">Memory of last 50 requests with full details.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 p-4">
            {filteredLogs.length === 0 ? (
                ...
                <div className="space-y-4">
                    {filteredLogs.map(log => (
                        <div key={log.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                            <div className="p-5">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        ...
                                        <div className="text-xs text-slate-500 flex flex-col gap-0.5 mt-1">
                                            <a href={`mailto:${log.customer.email}`} className="hover:text-emerald-600">{log.customer.email}</a>
                                            <a href={`tel:${log.customer.phone}`} className="hover:text-emerald-600">{log.customer.phone}</a>
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        >
                                            {expandedLogId === log.id ? 'Hide Details' : 'View Technical Data'}
                            ...
                            {expandedLogId === log.id && (
                                <div className="bg-slate-50 border-t border-slate-200 p-4 text-xs font-mono text-slate-600 space-y-2 animate-in slide-in-from-top-2 duration-200">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <span className="block font-bold text-slate-400 text-[9px] uppercase mb-0.5">IP Address</span>
                                            {log.metadata.ip}
                                        </div>
                                        <div>
                                            <span className="block font-bold text-slate-400 text-[9px] uppercase mb-0.5">Timezone</span>
                                            <span className="flex items-center gap-1.5"><Clock className="w-3 h-3" /> {log.metadata.timezone}</span>
                                        </div>
                                        <div className="md:col-span-2">
                                            <span className="block font-bold text-slate-400 text-[9px] uppercase mb-0.5">Source URL</span>
                                            <div className="truncate flex items-center gap-1.5" title={log.metadata.landingPage}>
                                                <Globe className="w-3 h-3" /> {log.metadata.landingPage}
                                            </div>
                                        </div>
                                        <div className="md:col-span-2">
                                            <span className="block font-bold text-slate-400 text-[9px] uppercase mb-0.5">User Agent</span>
                                    ...
                                    {log.tripSnapshot && (
                                        <div className="mt-4 pt-4 border-t border-slate-200">
                                            <span className="block font-bold text-slate-500 text-[10px] uppercase mb-1">Archived Trip Data Snapshot</span>
                                            <div className="text-[10px] text-slate-500">
                                                ID: {log.tripSnapshot.id} | Pax: {log.tripSnapshot.groupSize} | Price: ${log.tripSnapshot.pricePerPerson} | Vibe: {log.tripSnapshot.vibe}
                                            </div>
                                        </div>
                                    )}
                                </div>
