  ...
  const getTripImage = (trip: TripRecap) => {
    ...
    const collections = {
      ...
      reno_fun: [
        ...
        'https://images.unsplash.com/photo-1583908064973-19614777d0a1?auto=format&fit=crop&q=80&w=800',
    ...
    if (
      ...
      text.includes('tahoe') ||
      text.includes('edgewood') ||
      text.includes('hyatt') ||
      text.includes('beach')
    ) {
      category = 'tahoe';
    } else if (
      text.includes('graeagle') ||
      text.includes('truckee') ||
      text.includes('cabin') ||
      text.includes('pines') ||
      text.includes('nakoma')
    ) {
      category = 'mountain';
    } else if (text.includes('monterey') || text.includes('carmel')) {
      category = 'coastal';
    } else if (
      trip.vibe === 'Bachelor Party' ||
...
set.length];
  ...
  const getTrackingLink = (name: string, type: 'course' | 'lodging') => {
    ...
    return `${url}${sep}utm_source=caddie_archive&utm_medium=referral&utm_campaign=trip_card`;
  };
      ...
      <div className="h-40 bg-slate-900 relative shrink-0 overflow-hidden">
        ...
        <img
          ...
          className={`w-full h-full object-cover transition-all duration-[2000ms] ease-out ${
            imageLoaded
              ? 'opacity-90 scale-100 group-hover/card:scale-110'
              : 'opacity-0 scale-105'
          }`}
        />
        ...
        <div className="absolute top-3 right-3 z-10">
          <span
            className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border shadow-sm backdrop-blur-sm ${
              badgeColors[trip.vibe] || 'bg-white/90 text-slate-800'
            }`}
          >
            {trip.vibe}
          </span>
        ...
        <div className="absolute bottom-4 left-5 right-5 text-white">
          <h3 className="font-extrabold text-xl leading-snug drop-shadow-sm mb-1 line-clamp-2">
            {trip.groupName}
          </h3>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col bg-white">
        <div className="p-6 pb-2">
          ...
          <div className={`grid gap-3 mb-6 ${trip.rounds > 0 ? 'grid-cols-4' : 'grid-cols-3'}`}>
            ...
            >
              <span className="text-[10px] font-bold text-emerald-600/70 uppercase tracking-wider mb-0.5">
                From
              </span>
              ...
              <span className="text-[9px] uppercase font-bold text-emerald-600/60 mt-0.5">
                /Person
              </span>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {trip.courses.map((c, i) => {
                  ...
                  if (link) {
                    return (
                      <a
                        ...
                        className="bg-white text-slate-600 px-2.5 py-1.5 rounded-lg text-xs font-semibold border border-slate-200 shadow-sm hover:border-emerald-400 hover:text-emerald-700 hover:shadow-md transition-all flex items-center gap-1"
                      >
                        {c}
                  ...
                  return (
                    <span
                      key={i}
                      className="bg-slate-50 text-slate-600 px-2.5 py-1.5 rounded-lg text-xs font-semibold border border-slate-200"
                    >
                      {c}
                    </span>
                  );
...
6">
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-2">
                {trip.highlights.map((h, i) => {
                  const link =
                    getTrackingLink(h, 'course') ||
                    getTrackingLink(h, 'lodging');
                  ...
                  if (link) {
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-xs font-medium text-slate-600 flex items-center gap-2 hover:text-emerald-700 hover:underline"
                      >
          ...
          {trip.whyItWorked && (
            <div
              className={`border rounded-xl p-5 mb-2 relative overflow-hidden ${
                trip.rounds > 0
            ...
            >
              <div className="absolute top-0 right-0 p-3 opacity-10 pointer-events-none">
                <Lightbulb
                  className={`w-16 h-16 ${
                    trip.rounds > 0 ? 'text-amber-600' : 'text-slate-400'
                  }`}
                />
              </div>
              ...
              <p
                className={`text-xs leading-relaxed font-medium pl-1 ${
                  trip.rounds > 0 ? 'text-amber-950/80' : 'text-slate-600'
                }`}
              >
                "{trip.whyItWorked}"
              </p>
          ...
          <div className="px-6 pb-6 pt-0 animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-6 flex items-start gap-4 shadow-sm">
              ...
              <div>
                ...
                <span className="text-sm font-bold text-slate-900">
                  {trip.logistics?.transportType || 'Transport Not Specified'}
                </span>
              </div>
            </div>
            ...
            <div className="relative pl-8 space-y-8 py-2 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100">
              {trip.dailyItinerary?.map((day, idx) => {
                const activityLink =
                  getTrackingLink(day.activity, 'course') ||
                  getTrackingLink(day.activity, 'lodging');

                const locationLink =
                  getTrackingLink(day.location, 'course') ||
                  getTrackingLink(day.location, 'lodging');
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <span className="text-sm font-bold text-slate-800 leading-tight">
                          ...
                          {activityLink ? (
                            ...
                            >
                              {day.activity}{' '}
                              <ArrowUpRight className="inline w-3 h-3 mb-0.5" />
                            </a>
                          ) : (
                            day.activity
                          )}
            ...
            {trip.logistics?.specialRequests?.length > 0 && (
              <div className="mt-8 bg-rose-50 rounded-xl p-5 border border-rose-100">
                <h4 className="text-[10px] font-bold uppercase text-rose-500 tracking-wider mb-3">
                  Special Requests
                </h4>
                ...
                <ul className="space-y-3">
                  {trip.logistics.specialRequests.map((req, i) => (
                    <li
                      key={i}
                      className="text-xs text-slate-700 flex items-start gap-3"
                    >
